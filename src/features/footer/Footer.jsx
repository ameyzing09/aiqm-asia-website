...
// Icon mapping for social links (SVG paths)
const SOCIAL_ICONS = {
  facebook:
  ...
  return (
    <footer className="bg-gray-900 dark:bg-black text-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-wrap justify-between gap-12 animate-pulse">
          ...
          <div className="flex flex-col">
            ...
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="h-4 w-28 bg-gray-700 rounded mb-2" />
            ))}
          </div>
          <div className="flex flex-col">
            <div className="h-6 w-24 bg-gray-700 rounded mb-4" />
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-3">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-8 w-16 bg-gray-700 rounded-lg" />
              ))}
            </div>
          </div>
        </div>
...
export function Footer() {
  const { data: global, isLoading } = useGlobal()

  if (isLoading) {
    return <FooterSkeleton />
  }

  // Build footer links from navigation labels
  ...
  const accreditations = global?.accreditations ? Object.values(global.accreditations) : []
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-wrap justify-between gap-12">
          ...
          <div className="flex flex-col w-full lg:w-auto">
            ...
            <p className="text-gray-400 mb-4 max-w-xs">{companyInfo.description}</p>
            <div className="flex gap-4">
              {socialLinks.map(social => {
                // Try name first (user-friendly), then id (Firebase key)
                const iconKey = social.name?.toLowerCase() || social.id?.toLowerCase()
                const iconPath = SOCIAL_ICONS[iconKey]
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-3">
              {accreditations.map((accreditation, index) => (
                <div key={index} className="px-4 py-2 bg-gray-800 rounded-lg text-sm">
                  {typeof accreditation === 'string'
                    ? accreditation
                    : accreditation.name || accreditation}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="border-t border-gray-800 mt-12 pt-8 text-center text-gray-400">
          <p>
            &copy; {companyInfo.copyrightYear} {companyInfo.name}. All rights reserved. | Privacy
