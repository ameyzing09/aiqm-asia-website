...
export function SyncValidator() {
  ...
  const { data: heroCertifications, isLoading: heroCertificationsLoading } =
    useHero('certifications')
  ...
  const totalLoading = [...dataHooks, ...heroPages, ...aboutSections, ...globalSections].filter(
    s => s.isLoading
  ).length
    ...
    <div className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-6">
      ...
      <div className="pt-4 border-t border-white/10">
        {totalZombie > 0 ? (
          ...
          <p className="text-xs text-gray-500 text-center">Scanning Firebase for zombie data...</p>
        ) : (
